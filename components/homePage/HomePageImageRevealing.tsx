...
const HomePageImageRevealing = () => {
  ...
  const yellowColorBaseForDeveloper = useRef<HTMLDivElement>(null);
  const clippathDMaskForDeveloperRef = useRef<SVGClipPathElement>(null);
  const clippathASMaskRef = useRef<SVGClipPathElement>(null);
  const clippathDeveloperMaskRef = useRef<SVGClipPathElement>(null);
  const pathDDeveloperRef = useRef<SVGPathElement>(null);
  const pathASRef = useRef<SVGPathElement>(null);
  const pathDeveloperRef = useRef<SVGPathElement>(null);
  //refs - designer
  const designerBaseForD = useRef<HTMLDivElement>(null);
  const designerBaseForAs = useRef<HTMLDivElement>(null);
  const designerBaseForDesigner = useRef<HTMLDivElement>(null);
  const clippathDMaskForDesignerRef = useRef<SVGClipPathElement>(null);
  const clippathAsMaskForDesignerRef = useRef<SVGClipPathElement>(null);
  const clippathDesignerMaskRef = useRef<SVGClipPathElement>(null);
  const pathDDesignerRef = useRef<SVGPathElement>(null);
  const pathAsDesignerRef = useRef<SVGPathElement>(null);
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      function mainTextAnimationWithFinalFullySeen(
        clipPath: SVGClipPathElement,
        imageBase: HTMLDivElement,
        path: SVGPathElement,
        durationMultiplier: number
      ...
      const allElementsMoveToBeInvisible: SVGClipPathElement[] = [
        ...
        clippathDMaskForDeveloperRef.current!,
        clippathDeveloperMaskRef.current!,
        clippathDMaskForDesignerRef.current!,
        clippathAsMaskForDesignerRef.current!,
      ...
      tl.add(
        ...
        //   mainTextAnimation(
        //     clippathDeveloperMaskRef.current!,
        //     yellowColorBaseForDeveloper.current!,
        ...
        //   textSliderFromRightAnimation(
        //     clippathDMaskForDesignerRef.current!,
        //     designerBaseForD.current!,
        //     pathDDesignerRef.current!,
        //     0.9
        //   )
        // )
        // .add(
        //   textSliderFromRightAnimation(
        //     clippathAsMaskForDesignerRef.current!,
        //     designerBaseForAs.current!,
        //     pathAsDesignerRef.current!,
        //     0.8
        //   ),
        //   "-=0.5"
        // )
        .add(
          mainTextAnimationWithFinalFullySeen(
            clippathDesignerMaskRef.current!,
            designerBaseForDesigner.current!,
